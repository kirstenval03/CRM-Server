...
router.get('/:boardId/columns', async (req, res) => {
    try {
      ...
      const board = await Board.findById(boardId).populate('columns.taskIds');
...
router.post('/:boardId/columns', async (req, res) => {
  try {
    const { boardId } = req.params; // Extract boardId from the request parameters
    const { title, indexPosition } = req.body; // Extract title and indexPosition from the request body
    ...
    const newColumn = new Column({ title, indexPosition });
...
router.put('/:boardId/columns/:columnId', async (req, res) => {
  try {
    ...
    const { title, indexPosition } = req.body; // Extract updated title and indexPosition from the request body
    ...
    if (indexPosition) {
      columnToUpdate.indexPosition = indexPosition;
    }
